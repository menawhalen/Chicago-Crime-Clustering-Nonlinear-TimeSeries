...
################ spatial
demo_neigh <- read.gal("neigh_index.GAL")
neigh_mat <- nb2mat(demo_neigh, style = "W", zero.policy = T)
diag(neigh_mat) <- 1
D1 <- 1-neigh_mat
colnames(D1) <- rownames(D1) <- comm_ar$community
D_geo <- as.dist(D1)

###########################################
### get deseasoned data
stl_district <- data.frame(index_week) %>% 
  select(week, community, rate) %>% 
  group_by(community) %>% 
  group_map(~ stl(ts(.x$rate, start = decimal_date(.x$week[1]), frequency = 52), "periodic"))

names(stl_district) <- comm_ar$community


### join seasonal est and data to get deseasoned = data - seasonal
deseasoned_index <- bind_rows(lapply(stl_district, function(x) pluck(x, "time.series")[,"seasonal"])) %>% 
  mutate(week = unique(index_week$week)) %>% 
  pivot_longer(cols = -week, names_to = "community", values_to = "seasonal_effect") %>% 
  left_join(index_week) %>% 
  mutate(deseasoned = abs(rate - seasonal_effect))


index_wide <- pivot_wider(deseasoned_index[c("week", "community", "deseasoned")], names_from = community, values_from = deseasoned)

#############################################
dist_measure <- list(before = list(dat = index_wide %>% dplyr::filter(week < ymd("2020-01-01")),
                                   geo = D_geo),
                     after20  = list(dat = index_wide %>% dplyr::filter(week > ymd("2019-12-31") & week < ymd("2021-01-01")),
                                     geo = D_geo),
                     after23  = list(dat = index_wide %>% dplyr::filter(week > ymd("2019-12-31") & week < ymd("2024-01-01")),
                                     geo = D_geo))

get_trend <- function(data, thres){
  ts_dat <- ts(data$dat[2:78], start = data$dat$week[1], frequency = 52)
  weeks <- data$dat$week
  trend_out <- apply(ts_dat, 2, trendsegment, th.const = thres)
  trend_df <- bind_cols(week = weeks, bind_rows(lapply(trend_out, function(x) ts = pluck(x, 2)), .id = "community"))
  no_cpt <- bind_cols(bind_rows(lapply(trend_out, function(x) ts = pluck(x, 3)), .id = "community")) %>% mutate(thres = thres)
...
before_cpt <- bind_rows(dist_measure$before$cpt_total) %>% 
  pivot_longer(cols = - thres, names_to = "community", values_to = "number_cpt") %>% 
  ggplot(aes(factor(thres), number_cpt, group = community)) +
  geom_line(alpha = 0.3) +
  ggtitle("Before")
...
pdf("final_images/index/no_cpt_threshold.pdf")
...
before_ave <- bind_rows(dist_measure$before$diff, .id = "thres") %>%
  pivot_longer(cols = -thres, names_to = "community", values_to = "ss") %>%
  group_by(thres) %>%
...
pdf("final_images/index/elbow_threshold.pdf")
...
###########################################################################
## Threshold done 
## clutsering performed
###########################################################################
find_alpha <- function(dist, geo){
  a_norm <- data.frame(choicealpha(dist, geo, range.alpha=seq(0, 1, 0.1), K= 3, graph = F)$Qnorm)[-c(1,11),] %>% 
...
for (i in 1:length(dist_measure)) {
  if (i == 1) {
    ...
    trend <- get_trend(dist_measure[[i]], thres = 1.8)
  }
  else {
    trend <- get_trend(dist_measure[[i]], thres = 1.9)
  }
  
  ts_trend <- trend$trend_dat[-1]
  names(ts_trend) <- names(index_wide)[-1]
  dist_measure[[i]]$trend_dat <- trend$trend_dat
  dist_measure[[i]]$cpt_total <- trend$cpt_total
  ## dtw2
  ndtw <- function(x, y, w){
  ...
  ## trend
  dist_measure[[i]]$eucl <- TSclust::diss(ts_trend, METHOD = "EUCL")
  dist_measure[[i]]$dtw <- TSclust::diss(ts_trend, METHOD = "DTWARP")
  ## dtw2
  dtw2_diss_t <- as.list(ts_trend) %>%
    expand.grid(., .) %>% 
    mutate(distance = map2(Var1, Var2, ndtw, 2)) %>%
    as_tibble()
  
  dtw2_diss_t <- cbind(dtw2_diss_t, expand.grid(name1 = colnames(ts_trend), name2 = colnames(ts_trend)))
  ...
  #### dtw1
  dtw1_diss_t <- as.list(ts_trend) %>%
    expand.grid(., .) %>% 
    mutate(distance = map2(Var1, Var2, ndtw, 1)) %>%
    as_tibble()
  
  dtw1_diss_t <- cbind(dtw1_diss_t, expand.grid(name1 = colnames(ts_trend), name2 = colnames(ts_trend)))
  ...
  #### dtw3 
  dtw3_diss_t <- as.list(ts_trend) %>%
    expand.grid(., .) %>% 
    mutate(distance = map2(Var1, Var2, ndtw, 3)) %>%
    as_tibble()
  
  dtw3_diss_t <- cbind(dtw3_diss_t, expand.grid(name1 = colnames(ts_trend), name2 = colnames(ts_trend)))
...
lapply(seq_along(clusters), function(i){
  pdf( paste0("final_images/index/", names(clusters)[i], "_dendro.pdf"))
...
lapply(seq_along(clusters)[-1], function(i){
  for (j in 2:4){
    plot <- bind_rows(before = bind_rows(lapply(clusters[[1]], function(y) cutree(y,j)), .id = "diss"), 
      ...
      pivot_longer(cols = -c(time, diss), names_to = "community", values_to = "cluster") %>% 
    ...
    ggsave(paste0("final_images/index/", names(clusters)[i], "_cluster", j,"_map.pdf"), width = 7, height = 7)
  }
})
...
cluster_info <- bind_rows(clust) %>% 
  pivot_longer(cols = -c(time, diss, cluster_size), names_to = "community", values_to = "cluster")

write_rds(dist_measure, "data/dist_measure_index_list.rds")
write_rds(cluster_info, "data/cluster_assignments_index.rds")
...
for (i in c("eucl", "dtw1", "dtw2", "dtw3")) {
  for (j in seq_along(names(dist_measure))) {
    clust_numb <- filter(cluster_info, cluster_size == 2, diss == i, time == names(dist_measure)[j]) %>% 
      ...
      rename(name = community, value = cluster) %>% 
      deframe()
...
pdf("final_images/index/silhouette_plots2thres.pdf")
...
pdf("final_images/index/silhouette_plots3thres.pdf")
...
for (i in c("eucl", "dtw1", "dtw2", "dtw3")) {
  for (j in seq_along(names(dist_measure))) {
    clust_numb <- filter(cluster_info, cluster_size == 4, diss == i, time == names(dist_measure)[j]) %>% 
      select(community, cluster) %>% 
      rename(name = community, value = cluster) %>% 
      deframe()
    assign(paste0(names(dist_measure)[j], "_", i), silhouette(clust_numb, dist_measure[[j]][[i]]))
...
pdf("final_images/index/silhouette_plots4.pdf")
...
within_error <- function(dataset, dist_trend, dist_geo, dist_type, time_period, cluster_num){
  ...
  for (m in cluster_num) {
    dist_clust <- unique(dplyr::filter(dataset, diss == dist_type & 
                                         time == time_period & cluster == m)$community)
    ...
    I_cluster[[m]] <- data.frame(community = dist_clust, cluster = m, I_normal = I_normal, I_removed = I_removed) %>% 
...
full2_cluster <- filter(cluster_info, cluster_size ==2) %>% 
  left_join(mutate(index_week, time = case_when(
    week < ymd("2020-01-01") ~ "before",
    #week > ymd("2019-12-31") & week < ymd("2021-01-01") ~ "after20",
    week > ymd("2020-12-31") & week < ymd("2022-01-01") ~ "after21",
    week > ymd("2019-12-31") & week < ymd("2024-01-01") ~ "after23")))


for (i in c("eucl", "dtw1", "dtw2", "dtw3")) {
  for (j in seq_along(names(dist_measure))) {
    full <- within_error(full2_cluster, dist_measure[[j]][[i]], dist_measure[[j]]$geo, i, names(dist_measure)[j], 1:2)
    ave <- bind_cols(lapply(full, function(x) sum(x$diff_sq)/length(x$community)))
    names(ave) <- c("1", "2")
...
##########################################
pdf("final_images/index/inertia_withinerror_ss_comm2_thres.pdf")
ave_cluster_inertia <- list(before = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_before, function (x) unique(x$I_normal))),
                                               ...
                                               #dtw4 = bind_cols(lapply(dtw4_within_before, function (x) unique(x$I_normal))),
                            ...
                            after20 = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_after20, function (x) unique(x$I_normal))),
                                                ...
                                                #dtw4 = bind_cols(lapply(dtw4_within_after20, function (x) unique(x$I_normal))),
                            ...
                            after23 = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_after23, function (x) unique(x$I_normal))),
                                                ...
                                                #dtw4 = bind_cols(lapply(dtw4_within_after22, function (x) unique(x$I_normal))),
...
full3_cluster <- filter(cluster_info, cluster_size ==3) %>% 
  left_join(mutate(index_week, time = case_when(
    ...
    week > ymd("2019-12-31") & week < ymd("2021-01-01") ~ "after20",
    #week > ymd("2020-12-31") & week < ymd("2022-01-01") ~ "after21",
    week > ymd("2019-12-31") & week < ymd("2024-01-01") ~ "after22")))
...
##########################################
pdf("final_images/index/inertia_withinerror_ss_comm3_thres.pdf")
ave_cluster_inertia <- list(before = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_before, function (x) unique(x$I_normal))),
                                               dtw2 = bind_cols(lapply(dtw2_within_before, function (x) unique(x$I_normal))),
                                               dtw3 = bind_cols(lapply(dtw3_within_before, function (x) unique(x$I_normal))),
                                               #dtw4 = bind_cols(lapply(dtw4_within_before, function (x) unique(x$I_normal))),
...
full4_cluster <- filter(cluster_info, cluster_size ==4) %>% 
  left_join(mutate(index_week, time = case_when(
    week < ymd("2020-01-01") ~ "before",
    week > ymd("2019-12-31") & week < ymd("2021-01-01") ~ "after20",
    # week > ymd("2020-12-31") & week < ymd("2022-01-01") ~ "after21",
...
for (i in c("eucl", "dtw1", "dtw2", "dtw3")) {
  for (j in seq_along(names(dist_measure))) {
    full <- within_error(full4_cluster, dist_measure[[j]][[i]], dist_measure[[j]]$geo, i, names(dist_measure)[j], 1:4)
    ave <- bind_cols(lapply(full, function(x) sum(x$diff_sq)/length(x$community)))
    names(ave) <- c("1", "2", "3", "4")
    assign(paste0(i, "_within_", names(dist_measure)[j]), full)
    assign(paste0(i, "_within_", names(dist_measure)[j], "_ave"), ave)
...
##########################################
pdf("final_images/index/inertia_withinerror_ss_comm4.pdf")
ave_cluster_inertia <- list(before = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_before, function (x) unique(x$I_normal))),
                                               dtw2 = bind_cols(lapply(dtw2_within_before, function (x) unique(x$I_normal))),
                                               dtw3 = bind_cols(lapply(dtw3_within_before, function (x) unique(x$I_normal))),
                                               #dtw4 = bind_cols(lapply(dtw4_within_before, function (x) unique(x$I_normal))),
                            ...
                            after23 = bind_rows(dtw1 = bind_cols(lapply(dtw1_within_after23, function (x) unique(x$I_normal))),
                                                ...
                                                dtw3 = bind_cols(lapply(dtw3_within_after23, function (x) unique(x$I_normal))),
                                                eucl = bind_cols(lapply(eucl_within_after23, function (x) unique(x$I_normal)))) %>% 
                              mutate(time = rep("after22", 4),
                                     diss = c("dtw1", "dtw2", "dtw3", "eucl")))
####inertia
bind_rows(ave_cluster_inertia) %>% 
  rename("1" = "...1", "2" = "...2","3" = "...3", "4" = "...4") %>% 
  pivot_longer(cols = 1:4, names_to = "cluster", values_to = "I_normal") %>% 
  mutate(cluster = factor(cluster),
         diss = factor(diss, labels = c("DTW1", "DTW2", "DTW3", "Euclidean")),
         time =  factor(time, levels = c("before", "after20", "after23"), labels =  c("2018-2019", "2020", "2020-2024"))) %>% 
  ggplot(aes(cluster, I_normal, fill = diss)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~time) +
  labs(title = "Inertia per Cluster by Distance Measure for 4 Clusters",
       fill = "Distance Measures",
       x = "Cluster",
       y = "Inertia")
...
bind_rows(eucl_within_before_ave, eucl_within_after20_ave, eucl_within_after23_ave,
          ...
          dtw2_within_before_ave, dtw2_within_after20_ave, dtw2_within_after23_ave,
          dtw3_within_before_ave, dtw3_within_after20_ave, dtw3_within_after23_ave) %>% 
  mutate(time = rep(c("before", "after20", "after22"), 4),
         diss = rep(c("eucl","dtw1", "dtw2", "dtw3"), each = 3)) %>% 
  pivot_longer(cols = 1:4, names_to = "cluster", values_to = "error") %>% 
  mutate(diss = factor(diss, labels = c("DTW1", "DTW2", "DTW3", "Euclidean")),
         time = factor(time, levels = c("before", "after20", "after23"), labels =  c("2018-2019", "2020", "2020-2023"))) %>% 
  ggplot(aes(cluster, error)) +
  geom_bar(aes(fill = diss), stat = "identity", position = "dodge") +
  facet_wrap(~time) +
  labs(title = "Average Within Cluster Variation for 4 Clusters",
       fill = "Distance Measures",
       x = "Cluster", 
       y = "Difference Between Inertia and Inertia Removed")
...
bind_rows(ave_cluster_inertia) %>% 
  ...
  geom_bar(stat = "identity", position = "dodge") +
  # coord_cartesian(ylim=c(0, 5)) +
  labs( title = "Sum of Squares by Distance Measure for 4 Clusters",
        x = "Distance Measures",
        y = "Sum of Squares Value",
        fill = "Time Period")
